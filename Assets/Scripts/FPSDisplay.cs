...
public class FPSDisplay : MonoBehaviour {
    ...
    void Update() {
		Display(averageFPSText, counter.AverageFPS);
		Display(highestFPSText, counter.HighestFPS);
		Display(lowestFPSText, counter.LowestFPS);
    }

	private void Display(Text label, int fps) {
		label.text = stringsFrom00To99[Mathf.Clamp(fps, 0, 99)];
		foreach (FPSColor fpsColor in fpsColors) {
			if (fps >= fpsColor.minimumFPS) {
